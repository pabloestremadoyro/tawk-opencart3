...
class ControllerExtensionModuleTawkto extends Controller {
    ...
    public function index() {
		...
		$data['condicion1'] = !is_array($customer) && $customer->isLogged();
		
		if ($data['condicion1']){
		$data['nombre'] = $customer->getFirstName() . ' ' . $customer->getLastName();
		$data['correo'] = $customer->getEmail();
		}
    ...
    private function getWidget() {
        ...
        $settings = $this->model_setting_setting->getSetting('module_tawkto', $storeId);
